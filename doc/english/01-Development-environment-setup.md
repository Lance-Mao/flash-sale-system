...
![gozeroflashsale](../chinese/images/1/gozeroflashsale.png)

## 4. Business Architecture Diagram

![gozeroflashsale](../chinese/images/1/flash-sale-system-service.png)
...
flashsale-log : The log collection uses the
...
``` shell
$ Create database flashsale_order; use flashsale_order; source deploy/sql/flashsale_order.sql;
$ Create database flashsale_payment; use flashsale_payment; source flashsale_payment.sql;
$ Create database flashsale_product; use flashsale_product; source deploy/sql/flashsale_product.sql;
$ Create database flashsale_usercenter; use flashsale_usercenter; source flashsale_usercenter.sql;
```
...
Then on the current page, Create index pattern->type flashsale-* -> Next Step -> select @timestamp->Create index pattern
...
- There is no topic in kafka to create a log: flashsale-log

  Solution: Go to kafka and create flashsale-log, restart filebeat, go-stash
...
- Internal kafka problem
  ...
  2) In the kafka container internal command line using consumer.sh consumption flashsale-log, another terminal command line with producer.sh to flashsale-log send messages, if the consumer can not receive, indicating that the kafka problem, docker logs -f kafka to see what the problem
