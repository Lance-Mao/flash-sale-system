...
This project address : <https://github.com/Mikaelemmmm/flash-sale-system>
...
flash-sale-system/app/order/cmd/mq/internal/listen directory code
...
The unified management in flash-sale-system/app/order/cmd/mq/internal/listen/listen.go, then in the main call listen.Mqs can get all mq together start
...
flash-sale-system/app/order/cmd/mq/internal/listen/asynqMqs.go is the defined asynq
...
flash-sale-system/app/order/cmd/mq/internal/listen/asynqMqs.go is the defined kq (kafka for go-queue)
...
To write the actual business, we'll go under flash-sale-system/app/order/cmd/mq/internal/listen/mqs, which is also categorized here for easy maintenance
...
import (
 ...
 "flashsale/common/asynqmq"
 "flashsale/common/xerr"
...
Look under the flash-sale-system/app/order/cmd/mq/internal/mqs/kq folder, because kq is not quite the same as asynq, it is itself managed using go-zero's Service, which has implemented the starter, stopper interface, so we add it to the / Users/seven/Developer/goenv/flash-sale-system/app/order/cmd/mq/internal/listen/kqMqs.go to define a go-queue service directly to the serviceGroup and give it to main to start. Our business code only needs to implement go-queue's Consumer to write our own business directly.

1) /Users/seven/Developer/goenv/flash-sale-system/app/order/cmd/mq/internal/listen/kqMqs.go
...
/Users/seven/Developer/goenv/flash-sale-system/app/order/cmd/mq/internal/mqs/kq/paymentUpdateStatus.go
...
About timed tasks, currently flash-sale-system is not used, here I also explain
...
In this section, we learned to use the message queue, delayed queue , kafka can be viewed through the management tools, as for asynq view webui in flash-sale-system/docker-compose-env.yml we have started a good asynqmon, directly using <http://127.0>. 0.1:8980 can be viewed
