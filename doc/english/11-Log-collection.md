...
⚠️  Create the topic for logging as follows: flashsale-log If you have already created it when building the system development environment, you can ignore it here
...
cd /opt/kafka/bin/
./kafka-topics.sh --create --zookeeper zookeeper:2181 --replication-factor 1 -partitions 1 --topic flashsale-log
```
...
Then in the current page, Create index pattern -> enter flashsale-* -> Next Step -> select @timestamp -> Create index pattern
