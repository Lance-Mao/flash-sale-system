...
![gozeroflashsale](./images/1/gozeroflashsale.png)

### 四、业务架构图
![gozeroflashsale](./images/1/flash-sale-system-service.png)
...
```shell
$ git clone https://github.com/Mikaelemmmm/flash-sale-system
$ go mod tidy
```
...
```shell
$ docker network create flash-sale-system_flashsale_net
...
```shell
$ ./kafka-topics.sh --create --bootstrap-server localhost:9094 --replication-factor 1 --partitions 1 --topic flashsale-log
...
创建数据库flashsale_order && 导入deploy/sql/flashsale_order.sql数据

创建数据库flashsale_payment && 导入deploy/sql/flashsale_payment.sql数据

创建数据库flashsale_product && 导入deploy/sql/flashsale_product.sql数据

创建数据库flashsale_usercenter && 导入flashsale_usercenter.sql数据
...
然后在当前页面，Create index pattern->输入flashsale-*  -> Next Step ->选择@timestamp->Create index pattern
...
- kafka中没有创建log的topic ： flashsale-log

  解决：去kafka创建 flashsale-log，重启filebeat、go-stash
...
- 内部kafka问题
  ...
  $ docker exec -it kafka /bin/sh
  $ cd /opt/kafka/bin
  $ ./kafka-console-consumer.sh --bootstrap-server kafka:9092 --topic flashsale-log 
  ```
  ...
  ​		  2）在kafka容器内部命令行使用consumer.sh消费flashsale-log，另外一个终端命令行用producer.sh给flashsale-log发送消息，如果consumer收不到，说明kafka出问题了，docker logs -f kafka看看什么问题
